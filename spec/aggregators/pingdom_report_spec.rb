...
describe "Pingdom Report" do 
	...
	it "parses details data and aggregates it correctly" do 
		...
		subject = Aggregators::Pingdom::Report.new([1,2,3], 1535756400, 1537398000)

		res = subject.aggregate(data)
		expect(res).to eq(summary)
	end

	it "fetches upstream data based on the configured checks" do 
		subject = Aggregators::Pingdom::Report.new([{name: 'a', id: 1}, { name: 'b', id: 2}, {name: 'c', id: 3 }], 1535756400, 1537398000)
